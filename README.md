...
> 
> **주제:** 유방암세포 현미경 이미지 input시 세포 종류별 세그멘테이션 마스크 및 의학적 판독문을 자동 생성하는 프로그램 제작. 이를 통해 관련 의료진의 업무를 보조할 수 있는 시스템 구성. 
...
### 1-1) 웹앱 (웹 상에서 실제 사용 가능합니다.)
 - HuggingFace 서버 상태에 따라 로딩시간이 오래 걸릴 수 있습니다.(5~10분 가량)
 - 새 탭에서 열어 두시면, 일정 시간 후 프로그램이 자동 로드됩니다.
 - 허깅페이스 페이지 하단의 'Download here'을 클릭해 테스트용 현미경 이미지 다운로드가 가능합니다.
...
- **진단의 효율성 및 정확성 향상:**
  1. 세그멘테이션 마스크를 통해 조직 검사 이미지의 세포들을 종류별로 구분하고, 의학적 판독문을 함께 제공함으로써 보다 정확하고 빠른 의료진의 진단을 도울 수 있음.
  
  2. 이를 통해 의료기관은 조직 검사 판독에 소요되는 비용을 줄일 수 있음. 뿐만 아니라, 치료가 필요한 환자는 검사 후 치료 시작까지의 시간을 단축해 암세포가 더 확산되기 전에 보다 빠르게 치료를 시작할 수 있음.

  3. 마지막으로, 축적된 의료 데이터를 기반으로 한 전문적 식견을 다양한 지역과 기관의 의료진에게 일관되게 제공함으로써, 의료기관이나 의료진에 따른 진단 편차를 줄이고 세포검사 진단의 정확성과 품질을 높일 수 있음.
...
- **공통:** 데이터 전처리, 모델 성능 분석
- **손혁재:** 데이터 분석, Streamlit 웹앱 구현, 세포별 분류 데이터 사용을 통한 판독문 생성 모델 성능개선 아이디어 제안
- **백기원:** 데이터 전처리 파이프라인 구축, 모델 훈련 및 내용 정리, 판독문 생성 모델 성능 개선 실행
