...
### 1-1) 웹앱 (HuggingFace 호스팅 중으로, 실제 사용이 가능합니다.)
...
- **의료진 업무 부담 경감 및 정확도 향상:** AI를 통해 세포조직의 현미경 이미지에서 세포를 세그멘이션하여 암세포, 정상세포, 면역세포 등을 구분하고, 자연어로 되어 있는 의학적 판독문을 같이 제공함으로써 조직검사 이미지를 활용한 의료진의 진단을 보조할 수 있음. 이로써 의료기관은 다수 환자의 세포검사 결과를 빠른 시간 내에 비용 효율적으로 도출하고, 환자가 보다 저렴한 비용으로 검사일로부터 빠른 시간 내 필요한 치료를 받을 수 있게 도울 수 있음. 뿐만 아니라, 다양한 의료기관과 의학 전문가로부터 축적된 데이터를 통한 의료적 식견(insight)을 다양한 지역, 기관의 의료진들에게 일관되게 제공함으로써 오진의 가능성을 낮출 수 있음.
